...
select to_char(sysdate, 'YYYY-MM-DD') -- ' ' 는 fm(Format Modle)
from dual;
...
select to_char(sysdate, 'd') -- 요일을 인덱스로
from dual;
...
select to_char(sysdate, 'DD "of" Month') -- 일반문자는 ""로 넣기
from dual;

select to_char(hire_date, 'fmDD Month RR') -- fill mode 너비를 맞추지않음
from employees;
...
select last_name, to_char(hire_date, 'YYYY.MM.DD'), 
    next_day(add_months(to_char(hire_date, 'YYYY.MM.DD'), 3), 2)
from employees;
-- 다른코드
...
select last_name, hire_date
from employees
where hire_date = to_date('Sep 21, 2005', 'fxMon  dd yy'); -- Format eXtract(잘못된 데이터를 거르기 위하여 형식을 맞는지 확인)
--------------------------------------------------------------
...
select last_name, job_id, (salary * 12 * nvl(commission_pct + 1, 1))
from employees;
-- 다른코드
...
select first_name, last_name, -- parameter값이 같으면 null, 다르면 첫번째 parameter 리턴
    nullif(length(first_name), length(last_name)) 
from employees;

select last_name, job_id, -- 처음으로 null이 아닌 값 리턴
...
select decode(salary, 'a', 1) -- 기본값을 적지않으면 null 리턴
from employees;

select decode(salary, 'a', 1, 0) -- 기본값 0 기입하여 0 리턴
from employees;

select decode(job_id, 1, 1) -- 기준값으로 데이터 타입을 맞추려고함
from employees; -- error, invalide number
...
select case salary when 1 then '1' -- 기준값 비교값 타입 일치 / 리턴값은 리턴값끼리 일치
                    when 2 then '2'
                    else '0'
        end garde
from employees;
...
-- 과제] 이름, 입사일, 입사요일을 월요일부터 요일순으로 조회하라.
select last_name, hire_date, to_char(hire_date, 'Day')
from employees
order by to_char(hire_date -1 , 'd'); -- 실패
-- 정답
...
select last_name, hire_date,
    case when hire_date < '2006/01/01' then to_char(1000000, 'fmL9,999,999')
        else to_char(100000, 'fmL999,999')
        end giftcard
from employees;