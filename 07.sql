...
-- 과제] Kochhar에게 보고하는 사원들의 이름, 직업을 조회하라.
...
where salary > (select salary
                from employees
                where last_name = 'King'); -- error *subqurey 리턴값은 1개여야한다.
...
select employee_id, last_name
from employees
where salary =any (select min(salary) -- any 단독사용 불가능 다른 연산자에 붙어서 사용
                    from employees    -- 결과값중 하나라도 true가 나오면 채택
                    group by department_id);
...
from employees
where salary <all (select salary -- <any가 될경우 9000 미만 / <all이 될경우 4200 미만
                    from employees
