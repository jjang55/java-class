...
select first_name, last_name, job_id, salary
from employees
where last_name = 'King'; -- 데이터는 대소문자 구별!
...
select first_name
from employees
where first_name like 'S%'; -- like의 비교값은 모두 문자
...
select last_name, hire_date
from employees
where hire_date like '2005%'; -- %는 wildcard이고 0개이상을 뜻한다

select last_name
from employees
where last_name like 'K___'; -- _는 임의 1글자를 말한다
...
select last_name, job_id
from employees
where job_id like 'I_\_%' escape '\'; -- 특수문자를 일반문자로 변경하는 escape문자는 설정가능

select last_name, job_id
from employees
where job_id like 'IT[_%' escape'[';

-- 과제] 직업에 _R이 포함이된 사원들의 이름, 직업을 조회하라.
select last_name, job_id
from employees
...
from employees
where manager_id = null; -- 예외) ||붙이기 연산자는 null을 null값으로 인지하지않음

select last_name, manager_id
from employees
where manager_id is null; -- is null(한 세트) : null값을 찾고싶을 경우(null이 값이 아님)
                          -- where 절이 true일때 where 절 실행
-----------------------------------------------------------
...
--      20번이나 50번 부서에서 일하는 사원의 이름, 월급, 부서번호를 조회하라.
...
select last_name, job_id
from employees
where job_id not in ('IT_PROG', 'SA_REP'); -- not : 구하는 값의 여집합
...
select last_name, department_id
from employees
order by 2 desc; -- 변수가 될경우 칼럼 index를 정함

select last_name, department_id dept_id
from employees
order by dept_id desc; -- 별명으로도 가능

select last_name
from employees
where department_id = 100
order by hire_date; -- table에 없는 column으로 정렬가능
